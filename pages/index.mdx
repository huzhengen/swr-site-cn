...
“SWR” 这个名字来自于 `stale-while-revalidate`。一种由 [HTTP RFC 5861](https://tools.ietf.org/html/rfc5861) 推广的 HTTP 缓存失效策略。SWR 是一种策略，它首先从缓存中返回数据（过期的），然后发送 fetch 请求（重新验证），最后得到最新数据。
