...
通过采用这种模式，你可以不必以命令的方式**获取**数据：开始请求、更新加载状态并返回最终结果。 相反，你的代码更具有声明性：你只需要指定组件使用什么数据即可。
...
传统上，我们在顶级组件中使用 `useEffect` 获取一次数据，然后通过 props 将其传递给子组件（注意，我们现在不处理错误状态）：
...
通常，我们需要将所有的数据获取都保存在顶级组件中，并为树深处的每个组件添加 props。如果我们给页面添加更多的数据依赖，代码将变得更加难以维护。
...
现在数据已**绑定**到需要该数据的组件上，并且所有组件都是相互**独立**的。 所有的父组件都不需要了解关于数据或数据传递的任何信息。他们只是渲染。现在代码更简单，更易于维护了。

最美妙的是只有**1个请求**发送到 API，因为它们使用相同的 SWR key，并且请求自动**重复数据删除**、**缓存**和**共享**。

而且，应用现在能够在 [用户 focus 或网络重连](/docs/revalidation) 时重新获取数据！ 这意味着当用户的笔记本电脑从睡眠状态唤醒或用户在切换浏览器标签页时，数据将自动刷新。
