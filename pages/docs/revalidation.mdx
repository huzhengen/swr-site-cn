...
<Callout>
  如果你想手动重新验证数据，请查看 <Link href="/docs/mutation"><a href="/docs/mutation">mutation</a></Link>。
</Callout>
...
这对于立即同步到最新状态非常有用。对于在 stale mobile tabs 或**休眠**的笔记本电脑等情况下刷新数据也很有帮助。
...
SWR 会为你提供自动重新获取数据的选项。这很**智能**，意味着只有与 hook 相关的组件**在屏幕上**时，才会重新获取。
...
你可以通过设置 [`refreshInterval`](/docs/options) 值来启用他：

```js
useSWR('/api/todos', fetcher, { refreshInterval: 1000 })
```

还有其他选项，例如 `refreshWhenHidden` 和 `refreshWhenOffline`。这两项默认都是禁用的，所以当网页不在屏幕上或没有网络连接时，SWR 不会 fetch。
