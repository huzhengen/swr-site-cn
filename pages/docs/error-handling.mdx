...
在出现错误时 SWR 使用 [指数退避算法](https://en.wikipedia.org/wiki/Exponential_backoff) 重试请求。该算法允许应用从错误中快速恢复，而不会浪费资源频繁地重试。

你还可以通过 [onErrorRetry](/docs/options#options) 选项覆盖该 behavior：
...
你总是可以响应性的在组件内部获取 `error` 对象。但如果你想要全局处理错误，通知 UI 显示一个 [toast](https://vercel.com/design/toast) 或者一个 [snackbar](https://material.io/components/snackbars)，或在某处报告它，比如 [Sentry](https://sentry.io)，可以用 [`onError`](/docs/options#options) 事件：
